...
test(`SET ACTORS -> ${mesageTest[0]}`, async () => {
    ...
    expect(response.body[0].movieActor.movieId).toBe(movieId)
    expect(response.body[0].movieActor.actorId).toBe(currentActor.id)
});  
...
test(`SET DIRECTORS -> ${mesageTest[0]}`, async () => {
    ...
    expect(response.body).toHaveLength(1)
    expect(response.body.length).toBe(1)
    expect(response.body[0].movieDirector.movieId).toBe(movieId)
    expect(response.body[0].movieDirector.directorId).toBe(currentDirector.id)
});  
...
test(`SET GENRES -> ${mesageTest[0]}`, async () => {
    ...
    expect(response.body).toHaveLength(1)
    expect(response.body.length).toBe(1)
    expect(response.body[0].movieGenre.movieId).toBe(movieId)
    expect(response.body[0].movieGenre.genreId).toBe(currentGenre.id)
});
